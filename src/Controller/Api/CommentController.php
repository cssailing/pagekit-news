...
class CommentController
{
    ...
    public function indexAction($filter = [], $post = 0, $page = 0, $limit = 0)
    {
        ...
        if ($post) {
            ...
            $hits = Posts::find((int) $post);
            $hits->hit = $hits->hit+1;
            $hits->save();
        } elseif (!$this->user->hasAccess('news: manage comments')) {
            App::abort(403, __('Insufficient user rights.'));
        }

        if (!$this->user->hasAccess('news: manage comments')) {
        ...
        if (preg_match('/^(created)\s(asc|desc)$/i', $order, $match)) {
            ...
            $order = [1 => 'created', 2 => App::module('news')->config('comments.order')];
        }
        ...
        foreach ($comments as $i => $comment) {
            ...
            if ($post && (!$p || !$p->hasAccess($this->user) || !($p->isPublished() || $this->user->hasAccess('news: manage comments')))) {
    ...
    public function saveAction($data, $id = 0)
    {
        if (!$id) {
            ...
            if (!$this->user->hasAccess('news: post comments')) {
                App::abort(403, __('Insufficient User Rights.'));
            }
            ...
            } elseif ($this->news->config('comments.require_email') && (!@$data['author'] || !@$data['email'])) {
        ...
        if (!$this->user->hasAccess('news: skip comment min idle')
            and $minidle = $this->news->config('comments.minidle')
        ...
        if (!@$data['post_id'] || !$post = Posts::where(['id' => $data['post_id']])->first() or !($this->user->hasAccess('news: manage comments') || $post->isCommentable() && $post->isPublished())) {
            App::abort(404, __('Post not found.'));
        }

        $approved_once = (boolean) Comment::where(['user_id' => $this->user->id, 'status' => Comment::STATUS_APPROVED])->first();
        $comment->status = $this->user->hasAccess('news: skip comment approval') ? Comment::STATUS_APPROVED : ($this->user->hasAccess('news: comment approval required once') && $approved_once ? Comment::STATUS_APPROVED : Comment::STATUS_PENDING);

        // check the max links rule
        if ($comment->status == Comment::STATUS_APPROVED && $this->news->config('comments.maxlinks') <= preg_match_all('/<a [^>]*href/i', @$data['content'])) {
