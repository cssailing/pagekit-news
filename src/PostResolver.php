...
class PostResolver implements ParamsResolverInterface
{
    const CACHE_KEY = 'news.routing';
    ...
    public function match(array $parameters = [])
    {
        ...
        if (!$id) {
            ...
            if (!$post = Posts::where(compact('slug'))->first()) {
                App::abort(404, 'Post not found.');
            }
    ...
    public function generate(array $parameters = [])
    {
        ...
        if (!isset($this->cacheEntries[$id])) {
            ...
            if (!$post = Posts::where(compact('id'))->first()) {
